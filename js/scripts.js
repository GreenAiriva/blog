...
function getTagFilterHref(tag) {
  ...
  return `${baseHref}${encodeURIComponent(normalizedTag)}`;
}
...
function initTags() {
  ...
  const tagLinks = tags
    .map((tag) => {
      ...
      const href = getTagFilterHref(normalizedTag);
      if (!href) {
        return '';
      }
      return `<a class="tag" href="${href}">#${normalizedTag}</a>`;
    })
    .filter(Boolean);
  ...
  container.innerHTML = tagLinks.join('');
}
